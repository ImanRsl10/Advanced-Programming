...
void System::handle_GET_commands(vector <string> command_details)
{
    ...
    if(!is_valid_cmd(command, TOTW, PLAYERS, STANDING, USERS_RANKING, MATCHES_IN_WEEK, SQUAD, SHOW_BUDGET))
        throw BAD_REQUEST;
...
string assemble_name(vector <string> splitted_name)
{
    string name;
...
void System::show_user_budget()
{
    if(admin_logged || !user_logged)
        throw PERMISSION_DENIED;
    current_user->show_budget();
}

void System::handle_POST_commands(vector <string> command_details)
{
    string command = command_details[1];
    if(!is_valid_cmd(command, SIGNUP, LOGIN, REGISTER_ADMIN, LOGOUT, SELL, BUY, SET_CAP,
    ...
    else if(command == SET_CAP)
        handle_set_captain(command_details);
...
{
    if(admin_logged || !user_logged || transfer_window_status == CLOSED)
        throw PERMISSION_DENIED;
...
{
    if(admin_logged || !user_logged || transfer_window_status == CLOSED ||
       current_user->is_transfers_complete())
        throw PERMISSION_DENIED;
    ...
    current_user->sell_player(mentioned_player);
...
void System::handle_set_captain(vector <string> command_details)
{
    if(admin_logged || !user_logged)
        throw PERMISSION_DENIED;

    string player_name = assemble_name({command_details.begin() + 4, command_details.end()});
    current_user->set_captain(player_name);
    print_message(OK);
}
